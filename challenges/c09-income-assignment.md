...
## TODO: Pivot the data, rename the columns
  pivot_longer(
    ...
    names_pattern = "(Estimate|Margin of Error).+([0-9]-person families)",
    values_drop_na = TRUE
  ) %>%
  rename(
...
df_data <-
  df_q4 %>%
  left_join(df_pop, by = c("Geography"))
```
...
- Document your observations here.
  - I cannot conclude that the larger a family gets the wider range of
    incomes as locational differences might affect that.
- I can conclude that certain areas have greater wealth disparity as
  across the board the confidence intervals are signifigantly wider on
  some counties.
  - …
- Can you confidently distinguish between household incomes in Suffolk
  county? Why or why not?
  - I cannot because they are all stacked on each other but I suppose
    that means they all are roughly the same value so maybe yes?
...
df_data %>%
  ...
  labs(
    ...
    title = "Standard Error vs Population by Family Size"
  ) +
  theme_minimal()
```

![](c09-income-assignment_files/figure-gfm/q7-task-1.png)<!-- --> whoaa
thats pretty
...
- What *overall* trend do you see between `SE` and population? Why might
  this trend exist?
  - As the population estimate increases standard error goes down. I
    would imagine this is because with higher population estimates you
    get more data and you get less errors with more data points.
- What does this *overall* trend tell you about the relative ease of
  studying small vs large counties?
  - Really small counties suck for collecting precise estimates because
    outliers really can sway data.
...
df_data %>%
  filter(category %in% c("2-person families", "4-person families")) %>%
  select(geographic_area_name, category, income_estimate) %>%
  pivot_wider(
    names_from = category,
    values_from = income_estimate
  ) %>%
  mutate(difference = `4-person families` - `2-person families`) %>%
  arrange(desc(difference)) %>%
  slice_head(n = 20) %>%
  ggplot(aes(x = reorder(geographic_area_name, difference), y = difference)) +
  geom_col() +
  coord_flip() +
  labs(
    x = "County",
    y = "Income Difference (4-person - 2-person families)",
    title = "Top 20 Counties by Income Gap Between 4-person and 2-person Families"
  )
```
...
- What is the difference between 2person and 4person families in their
  income earned?

- generally small counties are showing very skewed data (look at Kent
  population north of 700 and its technically the biggest difference but
  its likely just outlines skewing the data)

- Really tiny counties aside generally the difference is about the same
  amount between the counties in the sample.
