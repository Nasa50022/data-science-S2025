...
df_sample_seq <- read_csv("helvig-seq.csv")
```
...
df_q3 <- df_sample_seq %>% mutate(last_name = str_extract(name, "\\w+$")) %>%
...
- Is this sample representative of *all* houses in Helvig? Why or why
  not?
  - No bc its a unrandomized sample of a small subset of houses.
...
- Which sample—sequential or random—is more *representative* of all
  ...
  - (Write your ‘why’ here.) Random works best because while technically
    not a perfect solution a random sample is much more representative
    of all homes vs a sequential chunk bc it draws from all over the
    range of houses vs a small range of 25.
...
- Did you find any highly prevalent names using `df_sample_random` that
  ...
  - (Your response here) Collins and Blomgren
- Is there any reason that people with the same last name might tend to
  *live near each other*?
  - (Your response here) Perhaps they are familially related and thus
    like living near each other to make it easy to visit?
...
name_prevalence <- function(df, last = "Collins") {
  df %>% 
    ...
    summarize(
      estimate = mean(last_name == last, na.rm = TRUE)
    ) %>%
...
df_interval_bootstrap <- 
  ...
  mutate(
    estimate = map(
      splits,
      function(split_df) {
        data <- analysis(split_df)
        name_prevalence(data, last = last_most)
      }
    )
  ) %>% 
...
- Note that we used the *random* sample with the bootstrap procedure in
  ...
  - (Yes or no.) No
  - (Write your ‘why’ here.) Because the sequential data is not
    representative of the total range of houses in helvig as we have
    established and thus the confidence interval will be woefully
    innacurate.
...
- (Possible measurement \#1) Money saved (\$3,966)
- (Possible measurement \#2) Occupation (Gardener)
- (Possible measurement \#3) Can consent? (FALSE)
