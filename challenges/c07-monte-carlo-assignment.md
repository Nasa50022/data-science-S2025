...
df_q3 <- 
  ...
  summarize(pi_est = mean(inside_circle) * 4)
df_q3
```
...
- What is a range of plausible values, based on the sampling
  ...
  - Just based of a visual examination excluding outliers its ranging
    from 3.15 to 3.65.
...
df_q6 <- 
  ...
  mutate(inside_circle = x^2 + y^2 <= 1) %>%
  summarize(
    pi_est = mean(inside_circle) * 4,
    se = sqrt(var(inside_circle * 4) / n()),
    pi_lo = pi_est - qt(0.975, df = n() - 1) * se,
    pi_up = pi_est + qt(0.975, df = n() - 1) * se
  )
df_q6
```
...
- How closely do your bootstrap CI and CLT CI agree?
  - They match pretty closely with at least the first 3 sig figs of each
    hi lo value matching its counterpart.
- Comment on the width of your CI(s). Would your estimate of $\pi$ be
  ...
  orbit)?
  - (Good enough as a rough estimate?) Yeah it seems like an acceptable
    amount of error for something as loose and impercise as a simple art
    project.
  - (Good enough as a precise estimate?) It is not precise for a rocket
    trajectory as the few percentage points of error from the method I
    used are way too much considering most aerospace applications need
    pi to be accurate to many decimal places.
- What would be a *valid* way to make your CI more narrow?
  - Maybe we could bump up the sample size to reduce the error which
    naturally tightens the confidence interval.
