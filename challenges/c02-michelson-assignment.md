...
measure the speed of light. His approach was a d?evelopment upon the
...
**Observations**: - Write your observations here! - Seems easy enough i
figured it out. - Why might your table differ from Michelson’s? -
Perhaps Michelsons numbers were a broader coarser estimate where he
rounded up way more then the computer’s numerical calcualtion?
...
## Your code here!
abs(LIGHTSPEED_VACUUM - LIGHTSPEED_MICHELSON) - LIGHTSPEED_PM ##abs to just show error magnitude not sign and then subtracting from his PM estimate to compare his uncertantiy against true error
```

    ## [1] 100.542

``` r
abs(LIGHTSPEED_VACUUM - LIGHTSPEED_MICHELSON) ##see prev
```

    ## [1] 151.542

**Observations**: - Is Michelson’s estimate of the error (his
uncertainty) greater or less than the true error? - His estimated error
is less then the true error. - Make a quantitative comparison between
Michelson’s uncertainty and his error. - The actual difference between
the true error of his estimate from reality and the calculated error
margin ins 100.542 km/s
...
**Observations**: Similarities - Generally both have the same average
value, Both have a wide spread of data. Differences - The simulated data
is a little more consistent then the real data. I think a little more of
the data points on the Sim graph are outliers but im not sure.
...
``` r
df_q2 %>% ggplot(aes(x=Temp,y=VelocityVacuum))+
  geom_line()+
  ggtitle("Speed of light in vacuum estimation as affected by Temperature")+
  xlab("Temperature (Deg F)")+
  ylab("Speed of light Vaccum (km/s)")
```

![](c02-michelson-assignment_files/figure-gfm/unnamed-chunk-2-1.png)<!-- -->

``` r
df_q2 %>% ggplot(aes(x=Distinctness,y=VelocityVacuum))+
  geom_point()+
  ggtitle("Speed of light in vacuum estimation as affected by data quality")+
  xlab("Data Quality (3 = good, 2 = fair, 1 = poor)")+
  ylab("Speed of light Vaccum (km/s)")
```
...
``` r
df_q2 %>% ggplot(aes(x=Temp,y=VelocityVacuum, color = Distinctness))+
  geom_point()+
  ggtitle("Speed of light in vacuum estimation as affected by Temperature")+
  xlab("Temperature (Deg F)")+
  ylab("Speed of light Vaccum (km/s)")
...
**Observations**: A few patterns that I noticed in the graphs that might
explain the difference between his estimate and the IRL speed of light
is that the lower quality data points tend to noticeably record a lower
speed vs the fair and high quality points.This is also reflected in the
very erratic temperature graph. This inaccuracy is very likely the
reason why his estimate is off from the actual lightspeed measurement.
