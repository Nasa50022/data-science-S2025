...
**Observations**: - The data looks very turbulent - has quartile data
too
...
df_psaap %>%
  ...
  theme_minimal() +
  labs(
    title = "Normalized Temperature Rise vs Channel Location",
    x = "Channel Location (x)",
    y = "Normalized Temperature Rise (T_norm)",
    color = "Simulation Index"
  ) +
...
calc_rmse <- function(model, data) {
  data %>%
    ...
    summarise(rmse = sqrt(mean((T_norm - pred)^2))) %>%
    pull(rmse)
}

rmse_baseline <- calc_rmse(fit_baseline, df_validate)
rmse_cheat <- calc_rmse(fit_cheat, df_validate)
rmse_nonphysical <- calc_rmse(fit_nonphysical, df_validate)
...
- Which model is *most accurate*? Which is *least accurate*?
  - the cheating model is the most accurate and the nonphysical is the
    least.
- What *Category* of variable is `avg_T`? Why is it such an effective
  predictor?
  - It is an output. Its such an effective predictior bc its cheating
    and using T_norm to calculate.
- Would we have access to `avg_T` if we were trying to predict a *new*
  value of `T_norm`? Is `avg_T` a valid predictor?
  - No we would not have acess in that case. Thus its a bad predictor
- What *Category* of variable is `idx`? Does it have any physical
  meaning?
  - Its a metadata varibale and no it does not have a meaning.
...
- Which columns are excluded in the model formula above? What categories
  ...
  the model?
  - idx,avg_q,avg_T,rms_T. Its important to leave them out bc they
    either are output values not available when making new cases or are
    metadata with no physical value
- Which inputs are *statistically significant*, according to the model?
  - Based on P values i would do x W L I_0 U_0 d_p and C_p
- What is the regression coefficient for `x`? What about the regression
  coefficient for `T_f`?
  - x 1.018 T_f -0.0003791
...
- How do these standard deviations relate to the regression coefficients
  for `x` and `T_f`?
  - They are related in the sense that they get multiplied together to
    represent the total change so both matter when figuring out the
    statistical signifigance of a number.
- Note that literally *all* of the inputs above have *some* effect on
  ...
  - To be honest the statistical signifigance is mostly abitrary as its
    all chosen by the alpha value and in situations like this we could
    give a physically important value too low of an importance in the
    model. Its essentially telling us not to trust these models too
    close.
...
- Which model tends to be more accurate? How can you tell from this
  predicted-vs-actual plot? Q4 is much more accurate because its a
  tighter cluster that shows something rather then scattered data vomit.
- Which model tends to be *more confident* in its predictions? Put
  differently, which model has *narrower prediction intervals*? again q4
  is more confident because it has much narrower intervals
- How many predictors does the `fit_simple` model need in order to make
  a prediction? What about your model `fit_q4`?
  - it only needs the channel location to predict. Q4 needs all the
    input variables except the excluded ones.
...
predictions <- df_design %>%
  add_uncertainties(fit_q6, interval = "prediction", level = pr_level, prefix = "pi")

validation_check <- df_validate %>%
  add_uncertainties(fit_q6, interval = "prediction", level = pr_level, prefix = "pi") %>%
  mutate(in_interval = T_norm >= pi_lwr & T_norm <= pi_upr) %>%
  summarise(
    fraction_in_interval = mean(in_interval),
    actual_coverage = mean(in_interval),
    target_coverage = pr_level
  )

dependable_range <- predictions %>%
  select(pi_lwr, pi_fit, pi_upr)

print("Dependable range for T_norm:")
```

    ## [1] "Dependable range for T_norm:"
...
``` r
print("Validation of prediction intervals:")
```

    ## [1] "Validation of prediction intervals:"
...
```
    ...
    ## # A tibble: 1 × 3
    ##   fraction_in_interval actual_coverage target_coverage
    ##                  <dbl>           <dbl>           <dbl>
    ## 1                0.933           0.933             0.8
...
- How much do you trust your model? Why?
  - I would reasonably trust this model as it overshot the 80%
    prediction interval by over 10 percent which indicates its being a
    little too cautious.
- What kind of interval—confidence or prediction—would you use for this
  task, and why?
  - i would go with a prediction interval because its a single instance
    and i need to deal with not just random variations but also
    uncertainty.
- What fraction of validation cases lie within the intervals you
  ...
  - Like the check stated 93.3 percent of cases make it in the
    intervals. This is way higher then the target of 80 percent which
    means like i said before its being more careful.
- What interval for `T_norm` would you recommend the design team to plan
  around?
  - I probably would chose an 80 percent interval centered on the
    predicted value of ~1.88 so about 1.46 to 2.3 ish?
- Are there any other recommendations you would provide?
  - Always always always try to validate with real world prototypes
    please.
