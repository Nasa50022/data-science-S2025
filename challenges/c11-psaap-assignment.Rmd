...
**Observations**:
- The data looks very turbulent
- has quartile data too
...
- Which model is *most accurate*? Which is *least accurate*?
  - the cheating model is the most accurate and the nonphysical is the least.
- What *Category* of variable is `avg_T`? Why is it such an effective predictor?
  - It is an output. Its such an effective predictior bc its cheating and using T_norm to calculate.
...
- Which columns are excluded in the model formula above? What categories do these belong to? Why are these important quantities to leave out of the model?
  - idx,avg_q,avg_T,rms_T. Its important to leave them out bc they either are output values not available when making new cases or are metadata with no physical value
...
- How do these standard deviations relate to the regression coefficients for `x` and `T_f`?
  - They are related in the sense that they get multiplied together to represent the total change so both matter when figuring out the statistical signifigance of a number.
- Note that literally *all* of the inputs above have *some* effect on the output `T_norm`; so they are all "significant" in that sense. What does this tell us about the limitations of statistical significance for interpreting regression coefficients?
  - To be honest the statistical signifigance is mostly abitrary as its all chosen by the alpha value and in situations like this we could give a physically important value too low of an importance in the model. Its essentially telling us not to trust these models too close.
...
Q4 is much more accurate because its a tighter cluster that shows something rather then scattered data vomit.
- Which model tends to be *more confident* in its predictions? Put differently, which model has *narrower prediction intervals*?
again q4 is more confident because it has much narrower intervals 
- How many predictors does the `fit_simple` model need in order to make a prediction? What about your model `fit_q4`?
  - it only needs the channel location to predict. Q4 needs all the input variables except the excluded ones.
...
- How much do you trust your model? Why?
  - I would reasonably trust this model as it overshot the 80% prediction interval by over 10 percent which indicates its being a little too cautious.
- What kind of interval---confidence or prediction---would you use for this task, and why?
  -  i would go with a prediction interval because its a single instance and i need to deal with not just random variations but also uncertainty.
- What fraction of validation cases lie within the intervals you predict? (NB. Make sure to calculate your intervals *based on the validation data*; don't just use one single interval!) How does this compare with `pr_level`?
  - Like the check stated 93.3 percent of cases make it in the intervals. This is way higher then the target of 80 percent which means like i said before its being more careful.
- What interval for `T_norm` would you recommend the design team to plan around?
  - I probably would chose an 80 percent interval centered on the predicted value of ~1.88 so about 1.46 to 2.3 ish?
- Are there any other recommendations you would provide?
  - Always always always try to validate with real world prototypes please.
